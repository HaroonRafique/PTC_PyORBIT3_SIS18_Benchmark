...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--profile", choices=("smoke", "reference"), default="smoke"); parser.add_argument("--particles", type=int); parser.add_argument("--turns", type=int); parser.add_argument("--output-dir", type=Path, default=STEP_DIR / "output"); parser.add_argument("--reference-root", type=Path); parser.add_argument("--skip-reference-comparison", action="store_true"); parser.add_argument("--madx", type=Path)
    args = parser.parse_args(argv); config = load_step_config(ROOT / "shared_inputs" / "benchmark_profiles.json", step=4, profile=args.profile); particles, turns = args.particles or config.n_macroparticles, args.turns or config.turns
