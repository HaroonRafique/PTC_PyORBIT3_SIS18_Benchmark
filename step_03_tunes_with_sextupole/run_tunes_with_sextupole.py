#!/usr/bin/env python
"""Run SIS18 Step 3: frozen-space-charge tunes with sextupole enabled."""
...
WEBSITE_REFERENCE_DIR = ROOT / "shared_inputs" / "reference_plots" / "step_03"
...
def scan_sigma_limit(plane: str) -> float:
    """Return the archival Step 3 launch limit for the selected transverse plane."""
...
def _plane_values(lattice, bunch, plane: str, particles: int) -> tuple[np.ndarray, np.ndarray, float]:
    ...
    coordinates = amplitude_scan_coordinates(
        plane=plane, n_particles=particles, n_sigma=scan_sigma_limit(plane), betax=lattice.betax0,
        betay=lattice.betay0, epsn_x=EPSN_X, epsn_y=EPSN_Y, beta_rel=bunch.getSyncParticle().beta(),
        gamma_rel=bunch.getSyncParticle().gamma(),
    )
    coordinate = coordinates[:, 0 if plane == "x" else 2]
    gamma_twiss = (1.0 + alpha * alpha) / beta
    return coordinates, np.column_stack((coordinate / sigma, np.sqrt(beta * gamma_twiss) * coordinate / sigma)), sigma


def _snapshot_by_id(bunch, particles: int) -> np.ndarray:
    """Return a fixed ID-indexed snapshot; removed particles remain NaN."""
...
def _extract_tunes(trajectories: np.ndarray, *, plane: str, analysis_turns: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    try:
        ...
        raise RuntimeError("Step 3 requires PyNAFF; install it with `python -m pip install PyNAFF`.") from error
...
def _write_table(path: Path, ids: np.ndarray, amplitudes: np.ndarray, pynaff: np.ndarray, fft: np.ndarray, bare_tune: float) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    for method, values in (("pynaff", pynaff), ("fft", fft)):
        method_path = path.with_name(f"{path.stem}_{method}{path.suffix}")
        with method_path.open("w", newline="", encoding="utf-8") as stream:
            ...
            for particle, tune in zip(ids, values):
                launch, effective = amplitudes[particle]
                writer.writerow((particle, f"{launch:.12g}", f"{effective:.12g}", f"{tune:.12g}", f"{_absolute_tunes(bare_tune, tune):.12g}"))
...
def _overlay_plot(path: Path, *, plane: str, legacy: np.ndarray, amplitudes: np.ndarray, ids: np.ndarray, pynaff: np.ndarray, fft: np.ndarray) -> Path:
    ...
    axis.set(xlabel=rf"effective {plane} amplitude [$\sigma_{plane}$]", ylabel=rf"fractional $Q_{plane}$", xlim=(0, 7) if plane == "x" else (0, 5), ylim=(.2325, .345) if plane == "x" else (.05, .20625))
    axis.set_box_aspect(1); axis.grid(True, alpha=0.3); axis.legend(markerscale=2)
    figure.savefig(path, dpi=160); plt.close(figure)
    return path
...
def _run_plane(*, flat: Path, inputs: Path, config, plane: str, particles: int, turns: int):
    ...
    for index in range(bunch.getSize()):
        ...
        bunch.partAttrValue("ParticleIdNumber", index, 0, index)
...
def main(argv: list[str] | None = None) -> int:
    ...
    plots, tables, trajectories_dir = output / "plots", output / "tables", output / "trajectories"; trajectories_dir.mkdir(parents=True, exist_ok=True)
    ...
    for plane, bare_tune in (("x", config.qx), ("y", config.qy)):
        trajectories, amplitudes, first_lost, summary = _run_plane(flat=flat, inputs=inputs, config=config, plane=plane, particles=particles, turns=turns)
        np.savez_compressed(trajectories_dir / f"{plane}_turn_by_particle.npz", coordinates=trajectories, turns=np.arange(turns + 1))
        ids, pynaff, fft = _extract_tunes(trajectories, plane=plane, analysis_turns=max(2, turns // 2 - 2))
        _write_table(tables / f"tunes_{plane}.csv", ids, amplitudes, pynaff, fft, bare_tune)
        with (tables / f"lost_particles_{plane}.csv").open("w", newline="", encoding="utf-8") as stream:
            writer = csv.writer(stream); writer.writerow(("particle_id", "launch_coordinate_sigma", "effective_amplitude_sigma", "first_lost_turn"))
            for particle_id in np.flatnonzero(first_lost >= 0): writer.writerow((particle_id, *amplitudes[particle_id], first_lost[particle_id]))
        current = _current_plot(plots / f"{plane}_tune_vs_launch_amplitude.png", plane=plane, amplitudes=amplitudes, ids=ids, pynaff=pynaff, fft=fft, bare_tune=bare_tune)
        current_plots[plane] = current; generated_plots.append(str(current)); summaries[plane] = {**summary, "surviving_tune_particles": len(ids), "lost_particles": int((first_lost >= 0).sum())}
        if reference is not None:
            reference_output = reference / "Step3" / "output"; legacy_table = reference_output / f"SIS18_Step3_{plane}_I=2.95e10_Tunes_OnData.txt"; legacy_plot = reference_output / f"SIS18_Step3_{plane}_I=2.95e10_Q{'x' if plane == 'x' else 'y'}.png"; legacy = np.loadtxt(legacy_table)
            comparison_payload[plane] = compare_tune_tables(legacy, np.column_stack((amplitudes[ids, 1], pynaff))).__dict__
            overlay = _overlay_plot(plots / f"legacy_vs_current_{plane}_numeric_overlay.png", plane=plane, legacy=legacy, amplitudes=amplitudes, ids=ids, pynaff=pynaff, fft=fft)
            side = plot_legacy_comparison(legacy_plot, current, plots / f"legacy_vs_current_{plane}_side_by_side.png", title=f"SIS18 Step 3 {plane}-plane tunes")
            generated_plots.extend((str(overlay), str(side))); reference_artifacts.update({str(legacy_table): sha256_file(legacy_table), str(legacy_plot): sha256_file(legacy_plot)})
